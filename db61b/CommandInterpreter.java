...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            ...
            if (_input.nextIs(";")) {
                return table1.select(table2, columnNames, null);
            }
            conditionList = conditionClause(table1, table2);
            return table1.select(table2, columnNames, conditionList);
        } else {
            if (_input.nextIs(";")) {
                return table1.select(columnNames, null);
            }
            conditionList = conditionClause(table1);
            return table1.select(columnNames, conditionList);
        }
    }
    ...
    Condition condition(Table... tables) {
        String colName = columnName();
        Column col1 = new Column(colName, tables);
        ...
        if (_input.nextIs(Tokenizer.LITERAL)) {
            ...
            colName = columnName();
            Column col2 = new Column(colName, tables);
